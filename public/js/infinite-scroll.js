...
let m = angular.module("kuInfiniteScroll", [
    "ngResource",
    "ngSanitize",
]);

// inject the dbNotes service factory into controller
m.controller("mainController", ["$timeout", "$window", function($timeout, $window) {
    let ctrl = this;
    ...
    ctrl.loadMore = () => {
        ...
        $timeout(function() {
            for (let i = 0; i < 20; i++) {
                ...
                const description = `I am ${i}. Date.now() is ${timestamp}`;
                ctrl.params.push({
                    timestamp,
                    description
                });
...
m.value('THROTTLE_MILLISECONDS', null);

m.directive('infiniteScroll', [
